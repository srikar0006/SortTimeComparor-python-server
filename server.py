...
class AlgorithmServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path not in ("/api/compare", "/compare"):
        ...
        try:
            ...
            if not numbers or not isinstance(numbers, list):
                self._send_json(400, {"error": "Please send a non-empty list of numbers."})
                return
...
if __name__ == "__main__":
    server = HTTPServer(("localhost", 8000), AlgorithmServer)
